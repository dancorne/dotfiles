...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ 'web', 'local', 'media', 'web', 'misc' }, s, layouts[2])
end
-- }}}
...
for s = 1, screen.count() do
    ...
    myvolumeicon = wibox.widget.textbox()
    myvolumeicon:set_markup(couth.alsa:getVolume('Master'))
    ...
    middle_layout:set_middle(mytextclock)

    -- Widgets that are aligned to the right
    local right_layout = wibox.layout.fixed.horizontal()
    if s == 1 then right_layout:add(wibox.widget.systray()) end
    --right_layout:add(myvolumeicon)
    ...
    layout:add(right_layout_aligned)
...
local complete = awful.completion.bashcomp_load("/usr/share/bash-completion/completions/ssh")

-- {{{ Key bindings
globalkeys = awful.util.table.join(
    awful.key({ }, "XF86AudioLowerVolume",   function () couth.notifier:notify( couth.alsa:setVolume('Master','4dB-')) end),
    ...
    awful.key({ modkey },       "F2",
            function ()
                    awful.prompt.run({ prompt = "SSH: ", completion_callback = complete  },
                    mypromptbox[mouse.screen].widget,
                    function (s)
                            awful.util.spawn("urxvt -e ssh " .. s)
                    end)
            end),
