...
starship_precmd() {
    ...
    if [[ $STARSHIP_START_TIME ]]; then
        ...
        PS1="$("/usr/local/bin/starship" prompt --status=$STARSHIP_CMD_STATUS --jobs="$NUM_JOBS" --cmd-duration=$STARSHIP_DURATION)"
        unset STARSHIP_START_TIME
    else
        PS1="$("/usr/local/bin/starship" prompt --status=$STARSHIP_CMD_STATUS --jobs="$NUM_JOBS")"
    fi
